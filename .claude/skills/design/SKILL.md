...
```
Agent 1: architect   → reads specs, writes to .claude/architecture/
Agent 2: ui-designer → reads specs, writes to .claude/design/
```

Both agents read from `.claude/specs/` but write to separate directories — no file conflicts.
...
3. **Spawn both agents concurrently using the Agent tool** (two Agent calls in one message):
   - Agent 1 (`architect`): "Read `.claude/skills/architecture/SKILL.md`. Read specs from `.claude/specs/`. Generate architecture outputs to `.claude/architecture/`."
   - Agent 2 (`ui-designer`): "Read `.claude/skills/ui-mockup/SKILL.md`. Read specs from `.claude/specs/`. Generate UI mockups to `.claude/design/`."
4. Architect writes to `.claude/architecture/`:
...
5. UI Designer writes to `.claude/design/`:
